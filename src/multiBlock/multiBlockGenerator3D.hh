...
{
    std::vector<MultiBlock3D::ProcessorStorage3D>
        newProcessors(from.getStoredProcessors());
    ...
    for (pluint iProcessor=0; iProcessor<newProcessors.size(); ++iProcessor) {
        ...
        if (newGenerator->extract(to.getBoundingBox())) {
            addInternalProcessor( *newGenerator,
                                  newProcessors[iProcessor].getMultiBlocks(),
...
{
    MultiBlockManagement3D intersectedBlocks (
            intersect ( originalField1.getMultiBlockManagement(),
                        originalField2.getMultiBlockManagement(), crop ) );
    ...
    return std::auto_ptr<MultiScalarField3D<T> > (
        new MultiScalarField3D<T> (
            block_union (
                ...
                originalField2.getMultiBlockManagement() ),
            originalField1.getBlockCommunicator().clone(),
            originalField1.getCombinedStatistics().clone(),
            defaultMultiBlockPolicy3D().getMultiScalarAccess<T>() )
    );
        ...
        MultiScalarField3D<T>& originalBlock, Box3D const& addedBlock )
{
    ...
    transferScalarFieldLocal( originalBlock, *newBlock,
...
MultiNTensorField3D<T>* generateMultiNTensorField3D(Box3D const& domain, plint ndim)
{
    ...
    MultiNTensorField3D<T>* field = new MultiNTensorField3D<T> (
        ndim,
        defaultMultiBlockPolicy3D().getMultiBlockManagement(domain,defaultEnvelopeWidth),
        defaultMultiBlockPolicy3D().getBlockCommunicator(),
        defaultMultiBlockPolicy3D().getCombinedStatistics(),
        defaultMultiBlockPolicy3D().getMultiNTensorAccess<T>() );
    return field;
...
MultiNTensorField3D<T>* clone (
        MultiNTensorField3D<T>& originalField,
        Box3D const& subDomain, bool crop )
{
...
MultiNTensorField3D<T1>*
    generateNTensorFieldFromBlockLattice3D (
            MultiBlockLattice3D<T2,Descriptor> const& lattice,
            Box3D const& intersection, plint nDim )
{
...
MultiNTensorField3D<T>* generateIntersectMultiNTensorField (
        MultiBlock3D const& originalField1,
        MultiBlock3D const& originalField2, plint nDim, bool crop )
{
    MultiNTensorField3D<T>* newField = new MultiNTensorField3D<T> (
            nDim,
            intersect(originalField1.getMultiBlockManagement(),
...
MultiNTensorField3D<T>* generateJoinMultiNTensorField (
        MultiBlock3D const& originalField1,
        MultiBlock3D const& originalField2, plint nDim )
{
    MultiNTensorField3D<T>* newField = new MultiNTensorField3D<T> (
            nDim,
            block_union (
...
template<typename T>
MultiNTensorField3D<T>* except (
        MultiNTensorField3D<T>& originalBlock,
        Box3D const& exceptedBlock )
{
    MultiNTensorField3D<T>* newBlock =
        new MultiNTensorField3D<T> (
        ...
        MultiTensorField3D<T,nDim>& originalBlock, Box3D const& addedBlock )
{
    ...
    transferTensorFieldLocal( originalBlock, *newBlock,
                              originalBlock.getBoundingBox() );
        ...
        Box3D const& subDomain, bool crop )
{
    ...
    transferBlockLatticeLocal( originalLattice, *clonedLattice,
                               originalLattice.getBoundingBox() );
    ...
    return std::auto_ptr<MultiBlockLattice3D<T,Descriptor> > (
        new MultiBlockLattice3D<T,Descriptor> (
            block_union (
                ...
                originalBlock2.getMultiBlockManagement() ),
            originalBlock1.getBlockCommunicator().clone(),
            originalBlock1.getCombinedStatistics().clone(),
            defaultMultiBlockPolicy3D().getMultiCellAccess<T,Descriptor>(),
            new NoDynamics<T,Descriptor> )
        ...
        MultiBlockLattice3D<T,Descriptor>& originalBlock, Box3D const& addedBlock )
{
    ...
    transferBlockLatticeLocal( originalBlock, *newBlock,
                               originalBlock.getBoundingBox() );
