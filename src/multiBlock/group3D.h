...
public:
    ...
    template<typename T>
    plint generateNTensor(std::string name="", plint nDim=1, plint envelopeWidth=1, plint gridLevel=0);
    template<typename T>
    plint generateNTensor( std::string name, plint nDim, std::vector<Box3D> const& bulks,
                           plint envelopeWidth, plint gridLevel, bool cropBoundingBox );
    ...
    template<typename T, template<typename U> class Descriptor>
    plint generateDenseParticles(std::string name="", plint envelopeWidth=1, plint gridLevel=0);
    plint generateContainer(std::string name="", plint envelopeWidth=1, plint gridLevel=0);
    plint generateContainerWithData(std::string name="", ContainerBlockData* data=0, plint envelopeWidth=1, plint gridLevel=0);
...
void addTransform (
        ...
        std::string name, TConv scalingFactor, TConv additiveOffset = (TConv) 0 );

template<typename T>
void addTransform (
        Group3D& group, std::auto_ptr<MultiScalarField3D<T> > field,
        std::string name, T scalingFactor, T additiveOffset = (T) 0 );
