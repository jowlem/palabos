...
GridLevelContainer3D<T,Descriptor>::GridLevelContainer3D(std::vector<Group3D*>& groups, plint order_, plint level_)
    ...
    dyn(dyn_), order(order_), level(level_), parity(0), owned(true)
{
    ...
    lattice = new MultiBlockLattice3D<T,Descriptor>(ogs.getMultiBlockManagement(level,1),
    ...
    if (level < ogs.getNumLevels()-1) {
        ...
        for (std::vector<boxLogic::DirectedPlane>::iterator it = coarseExt.begin(); it != coarseExt.end(); ++it) {
        ...
        MultiBlockManagement3D managementFine = ogs.getMultiBlockManagement(level+1,envelopeFine);

        MultiBlockManagement3D coarseManagement = align( finalCoarseBoxes,
                                                         managementCoarse, 2, level, true );    
        MultiBlockManagement3D fineManagement   = align( finalFineBoxes,
                                                         managementFine, envelopeFine, level+1, true );
...
template<typename T, template <typename U> class Descriptor>
GridLevelContainer3D<T,Descriptor>::GridLevelContainer3D(GridLevelContainer3D<T,Descriptor> const& rhs)
    : dyn(rhs.dyn), order(rhs.order),level(rhs.level), numVariables(rhs.numVariables),
        lattice(rhs.lattice), decomposed_t0(rhs.decomposed_t0), decomposed_t12(rhs.decomposed_t12), 
        decomposed_t1(rhs.decomposed_t1), decomposed_fine(rhs.decomposed_fine), decomposedContainer(rhs.decomposedContainer),
        parity(rhs.parity), owned(rhs.owned)
{ }

template<typename T, template<typename U> class Descriptor>
GridLevelContainer3D<T,Descriptor>&
...
template<typename T, template <typename U> class Descriptor, 
    template<typename T2, template<typename U2> class Descriptor2> class Engine>
MultiLevelCoupling3D<T,Descriptor,Engine>::MultiLevelCoupling3D(MultiLevelCoupling3D<T,Descriptor,Engine> const& rhs)
    : ogs(rhs.ogs), dyn(rhs.dyn), order(rhs.order), 
        overlapWidth(rhs.overlapWidth),
        gridLevels(rhs.gridLevels), couplings(rhs.couplings), filterAll(rhs.filterAll)
{ }

template<typename T, template <typename U> class Descriptor, 
    template<typename T2, template<typename U2> class Descriptor2> class Engine>
MultiLevelCoupling3D<T,Descriptor,Engine>&
    MultiLevelCoupling3D<T,Descriptor,Engine>::operator=(MultiLevelCoupling3D<T,Descriptor,Engine> const& rhs)
{
...
template<typename T, template <typename U> class Descriptor, 
    template<typename T2, template<typename U2> class Descriptor2> class Engine>
void MultiLevelCoupling3D<T,Descriptor,Engine>::swap(MultiLevelCoupling3D<T,Descriptor,Engine>& rhs)
{
...
void MultiLevelCoupling3D<T,Descriptor,Engine>::initializeTensorFields()
{
    ...
    for (plint iL = 0; iL < (plint)gridLevels.size()-1; ++iL) {
        ...
        for (pluint iA = 0; iA < cToFext_cu.size(); ++iA) {
            applyProcessingFunctional(new DecomposeAndRescaleFunctional3D<T,Descriptor,Engine>((T)0.5,T(), gridLevels[iL]->getOrder()),
        ...
        for (pluint iA = 0; iA < fToCint_fu.size(); ++iA) {
            applyProcessingFunctional (
                    ...
                    fToCint_fu[iA], gridLevels[iL+1]->getLattice(), gridLevels[iL]->getDecomposedFine() );
        }
    }
}
...
template<typename T, template <typename U> class Descriptor, 
    template<typename T2, template<typename U2> class Descriptor2> class Engine>
void MultiLevelCoupling3D<T,Descriptor,Engine>::
            ...
            CouplingInterfaces3D &coupling, plint numExec )
{
    ...
    for (pluint iA = 0; iA < coupling.getCtoFplanes_cu().size(); ++iA) {
        ...
        integrateProcessingFunctional(new CopyAndSpatialInterpolationPlaneFunctional3D<T>(dir), bb, cTensor, fTensor, numExec);
    }
    ...
    for (pluint iA = 0; iA < coupling.getCtoFbulkEdges_cu().size(); ++iA) {
        ...
        integrateProcessingFunctional(new CopyAndSpatialInterpolationEdgeFunctional3D<T>(planeDir,dir1,dir2), bb, cTensor, fTensor, numExec);
    }
    ...
    for (pluint iA = 0; iA < coupling.getCtoFbulkCorners_cu().size(); ++iA) {
        ...
        integrateProcessingFunctional(new CopyAndSpatialInterpolationCornerFunctional3D<T>(planeDir,dir1,dir2), bb, cTensor, fTensor, numExec);
    }
    ...
    for (pluint iA = 0; iA < coupling.getCtoFboundaryEdges_cu().size(); ++iA) {
        ...
        integrateProcessingFunctional(new CopyAndSpatialInterpolationBoundaryEdgeFunctional3D<T>(planeDir,dir1,dir2), bb, cTensor, fTensor, numExec);
    }
    ...
    for (pluint iA = 0; iA < coupling.getCtoFboundaryCorners_cu().size(); ++iA) {
        ...
        integrateProcessingFunctional(new CopyAndSpatialInterpolationBoundaryCornerFunctional3D<T>(planeDir,dir1,dir2), bb, cTensor, fTensor, numExec);
    }
}
    ...
    if (iL < (plint)(gridLevels.size()-1)) {
        ...
        for (plint iA = 0; iA < (plint)cToFext.size(); ++iA) { 
            ...
            integrateProcessingFunctional (
                    new DecomposeAndRescaleFunctional3D<T,Descriptor,Engine> (
                        (T)0.5,T(), gridLevels[iL]->getOrder() ),
                    cToFext[iA].bb, gridLevels[iL]->getLattice(), gridLevels[iL]->getDecomposedT0(), lattice_decomposeT0);
        }
        ...
        for (pluint iA = 0; iA < cToFext.size(); ++iA) {
            ...
            integrateProcessingFunctional (
                    new TemporalInterpolationFunctional3D<T>(), cToFext[iA].bb,
                    swappedArg, container_timeInterpolation );
        }

        integrateCopyAndSpatialInterpolation (
                gridLevels[iL]->getDecomposedT12(),
                gridLevels[iL]->getDecomposedFine(), *couplings[iL], decomposedT12_spatialInterpolation);
        
        std::vector<boxLogic::Plane> cToFint = couplings[iL]->getCtoFint_fu();
        for (pluint iA = 0; iA < cToFint.size(); ++iA) {
            integrateProcessingFunctional (
                    new RecomposeFunctional3D<T,Descriptor> (
                        gridLevels[iL+1]->getOrder() ),
                    cToFint[iA].bb, gridLevels[iL+1]->getLattice(),
                    gridLevels[iL]->getDecomposedFine(), lattice_recomposeFine );
        }

        integrateCopyAndSpatialInterpolation (
                gridLevels[iL]->getDecomposedT0(),
                gridLevels[iL]->getDecomposedFine(), *couplings[iL], decomposedT1_spatialInterpolation );
        integrateCopyAndSpatialInterpolation (
                gridLevels[iL]->getDecomposedT1(),
                gridLevels[iL]->getDecomposedFine(), *couplings[iL], decomposedT1_spatialInterpolation );
        ...
        for (pluint iA = 0; iA < fToCint_fu.size(); ++iA) {
            integrateProcessingFunctional (
                    new DecomposeAndRescaleFunctional3D<T,Descriptor,Engine> (
                        (T)2,T(), gridLevels[iL+1]->getOrder() ),
                    fToCint_fu[iA], gridLevels[iL+1]->getLattice(), gridLevels[iL]->getDecomposedFine(), lattice_decomposeFine );
        }
        
        std::vector<boxLogic::Plane> fToCint_cu = couplings[iL]->getFtoCint_cu();
        for (pluint iA = 0; iA < fToCint_cu.size(); ++iA) {
            integrateProcessingFunctional (
                    new CopyAndFilterNonEquilibriumFunctional3D<T,Descriptor>(filterAll), fToCint_cu[iA].bb,
                    gridLevels[iL]->getDecomposedT12(), gridLevels[iL]->getDecomposedFine(), decomposedT12_copyAndFilter );
        }
        ...
        for (pluint iA = 0; iA < fToCbcEdges_cu.size(); ++iA) {
            integrateProcessingFunctional (
                    ...
                    gridLevels[iL]->getDecomposedT12(), gridLevels[iL]->getDecomposedFine(), decomposedT12_copyBc );
        }
        
        for (pluint iA = 0; iA < fToCint_cu.size(); ++iA) {
            integrateProcessingFunctional (
                    new RecomposeFunctional3D<T,Descriptor>(gridLevels[iL]->getOrder()), fToCint_cu[iA].bb,
                    gridLevels[iL]->getLattice(), gridLevels[iL]->getDecomposedT12(), lattice_recomposeCoarse );
        }
        
        for (pluint iA = 0; iA < fToCbcEdges_cu.size(); ++iA) {
            integrateProcessingFunctional (
                    new RecomposeFunctional3D<T,Descriptor>(gridLevels[iL]->getOrder()),
                    fToCbcEdges_cu[iA].bb, gridLevels[iL]->getLattice(), gridLevels[iL]->getDecomposedT12(),
                    lattice_recomposeBc );
        }
    }
}
                                                                 ...
                                                                 ) 
{
    ...
    if (iL < (plint)(gridLevels.size()-1)) {
        ...
        gridLevels[iL]->getDecomposedContainer()[0]->executeInternalProcessors(container_timeInterpolation, false);
        ...
        collideAndStream(iL+1, useExecuteInternalProcessors, extProcIds, computeStats, statsId, ids, results);  // collision fine
        global::timer("gr_coupling_"+util::val2str(iL)).start();
        global::timer("gr_coupling").start();
        ...
        gridLevels[iL]->getDecomposedT12().executeInternalProcessors(decomposedT12_spatialInterpolation);
        ...
        if (computeStats) {
            ...
            for (pluint iA = 0; iA < ids[iL+1].size(); ++iA) {
                results[iL+1][iA].push_back(gridLevels[iL+1]->getLattice().getInternalStatistics().getSum(ids[iL+1][iA]));
            }
        }
        ...
        global::timer("gr_copyAndSpatialInterpolation_"+util::val2str(iL)).start();
        global::timer("gr_copyAndSpatialInterpolation").start();
        gridLevels[iL]->getDecomposedT1().executeInternalProcessors(decomposedT1_spatialInterpolation);
        global::timer("gr_copyAndSpatialInterpolation").stop();
        global::timer("gr_copyAndSpatialInterpolation_"+util::val2str(iL)).stop();
        ...
        if (iL == 0) {
            ...
            if (computeStats) {
                ...
                for (pluint iA = 0; iA < ids[iL].size(); ++iA) {
                    results[iL][iA].push_back(gridLevels[iL]->getLattice().getInternalStatistics().getSum(ids[iL][iA]));
                }
            }
        }
