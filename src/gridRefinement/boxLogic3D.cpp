...
std::vector<DirectedPlane> getDirectedPlanes(
    const Box3D &box, const Array<bool,26> &neighbor, const Array<bool,26> &bcNeighbor, const Array<bool,26> &allocNeighbor)
{
...
std::vector<DirectedEdge> getDirectedEdges(
    const Box3D &box,
    const Array<bool,26> &neighbor,
    const Array<bool,26> &bcNeighbor, const Array<bool,26> &allocNeighbor)
{
    ...
    if (neighbor[OT::surface0N()] && neighbor[OT::surface1P()] && (!neighbor[OT::edge2NP()] && allocNeighbor[OT::edge2NP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0, box.x0, box.y1, box.y1, box.z0+1+dz0, box.z1-1-dz1),0,-1, 0));
    ...
    if (neighbor[OT::surface0P()] && neighbor[OT::surface1N()] && (!neighbor[OT::edge2PN()] && allocNeighbor[OT::edge2PN()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y0, box.y0, box.z0+1+dz0, box.z1-1-dz1),0,+1, 0));
    ...
    if (neighbor[OT::surface0P()] && neighbor[OT::surface1P()] && (!neighbor[OT::edge2PP()] && allocNeighbor[OT::edge2PP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y1, box.y1, box.z0+1+dz0, box.z1-1-dz1),0,-1, 0));
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y1, box.y1, box.z0+1+dz0, box.z1-1-dz1),1, 0,-1));
    }
    ...
    if (neighbor[OT::surface0N()] && neighbor[OT::surface2N()] && (!neighbor[OT::edge1NN()] && allocNeighbor[OT::edge1NN()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0, box.x0, box.y0+1+dy0, box.y1-1-dy1, box.z0, box.z0),0, 0,+1));
    ...
    if (neighbor[OT::surface0N()] && neighbor[OT::surface2P()] && (!neighbor[OT::edge1PN()] && allocNeighbor[OT::edge1PN()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0, box.x0, box.y0+1+dy0, box.y1-1-dy1, box.z1, box.z1),0, 0,-1));
    ...
    if (neighbor[OT::surface0P()] && neighbor[OT::surface2N()] && (!neighbor[OT::edge1NP()] && allocNeighbor[OT::edge1NP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y0+1+dy0, box.y1-1-dy1, box.z0, box.z0),0, 0,+1));
    ...
    if (neighbor[OT::surface0P()] && neighbor[OT::surface2P()] && (!neighbor[OT::edge1PP()] && allocNeighbor[OT::edge1PP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y0+1+dy0, box.y1-1-dy1, box.z1, box.z1),0, 0,-1));
        edges.push_back(DirectedEdge(Box3D(box.x1, box.x1, box.y0+1+dy0, box.y1-1-dy1, box.z1, box.z1),2,-1, 0));
    }
    ...
    if (neighbor[OT::surface1N()] && neighbor[OT::surface2N()] && (!neighbor[OT::edge0NN()] && allocNeighbor[OT::edge0NN()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0+1+dx0, box.x1-1-dx1, box.y0, box.y0, box.z0, box.z0),1,+1, 0));
    ...
    if (neighbor[OT::surface1N()] && neighbor[OT::surface2P()] && (!neighbor[OT::edge0NP()] && allocNeighbor[OT::edge0NP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0+1+dx0, box.x1-1-dx1, box.y0, box.y0, box.z1, box.z1),1,-1, 0));
    ...
    if (neighbor[OT::surface1P()] && neighbor[OT::surface2N()] && (!neighbor[OT::edge0PN()] && allocNeighbor[OT::edge0PN()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0+1+dx0, box.x1-1-dx1, box.y1, box.y1, box.z0, box.z0),1,+1, 0));
    ...
    if (neighbor[OT::surface1P()] && neighbor[OT::surface2P()] && (!neighbor[OT::edge0PP()] && allocNeighbor[OT::edge0PP()])) {
        ...
        edges.push_back(DirectedEdge(Box3D(box.x0+1+dx0, box.x1-1-dx1, box.y1, box.y1, box.z1, box.z1),1,-1, 0));
        edges.push_back(DirectedEdge(Box3D(box.x0+1+dx0, box.x1-1-dx1, box.y1, box.y1, box.z1, box.z1),2, 0,-1));
    }
...
std::vector<DirectedCorner> getBoundaryDirectedCorners(
    const Box3D &box, const Array<bool,26> &neighbor,
    const Array<bool,26> &bcNeighbor, const Array<bool,26> &allocNeighbor)
{
    std::vector<DirectedCorner> corners;
...
void getInterfaces (
    const Box3D &box,
    const Array<bool, 26> &neighbor,
    const Array<bool, 26> &bcNeighbor,
    const Array<bool, 26> &allocNeighbor,
    plint overlapWidthCoarseUnits,
    std::vector<boxLogic::Plane> &coarseToFinePlanesFineUnits,             // used for recompose of   fine interface
    std::vector<boxLogic::DirectedPlane> &coarseToFinePlanesCoarseUnits,   // used for interpolation Processing Functionals (PF)
    std::vector<boxLogic::DirectedEdge> &coarseToFineEdgesCoarseUnits,     // used for interpolation PF
    std::vector<boxLogic::DirectedCorner> &coarseToFineCornersCoarseUnits, // used for interpolation PF
    std::vector<boxLogic::DirectedPlane> &coarseToFinePlanesCoarseUnitsExtended,   // used for decompose of coarse interface
    std::vector<Box3D> &fineToCoarsePlanesFineUnits,             // used for decompose of fine interfare
    std::vector<boxLogic::Plane> &fineToCoarsePlanesCoarseUnits,            // used for filtering and recompose of coarse interface
    ...
    )
{
    ...
    Box3D coarseToFineBoxFineUnits   =
        Box3D(box.x0,box.x1-!(neighbor[OT::surface0P()]),
              box.y0,box.y1-!(neighbor[OT::surface1P()]),
              box.z0,box.z1-!(neighbor[OT::surface2P()]));
    ...
    for (plint iA = 0; iA < (plint)coarseToFinePlanesCoarseUnitsTmp.size(); ++iA) {
        ...
        nCellsOne = Array<plint,2>(2*(!neighbor[dirOne.first] && !bcNeighbor[dirOne.first])+bcNeighbor[dirOne.first],
                                   2*(!neighbor[dirOne.second] && !bcNeighbor[dirOne.second])+bcNeighbor[dirOne.second]);

        nCellsTwo = Array<plint,2>(2*(!neighbor[dirTwo.first] && !bcNeighbor[dirTwo.first])+bcNeighbor[dirTwo.first],
    ...
    plint dx0 = ((!neighbor[OT::surface0N()] && allocNeighbor[OT::surface0N()])) - bcNeighbor[OT::surface0N()];
    plint dx1 = ((!neighbor[OT::surface0P()] && allocNeighbor[OT::surface0P()])) - bcNeighbor[OT::surface0P()];
    plint dy0 = ((!neighbor[OT::surface1N()] && allocNeighbor[OT::surface1N()])) - bcNeighbor[OT::surface1N()];
    plint dy1 = ((!neighbor[OT::surface1P()] && allocNeighbor[OT::surface1P()])) - bcNeighbor[OT::surface1P()];
    plint dz0 = ((!neighbor[OT::surface2N()] && allocNeighbor[OT::surface2N()])) - bcNeighbor[OT::surface2N()];
    plint dz1 = ((!neighbor[OT::surface2P()] && allocNeighbor[OT::surface2P()])) - bcNeighbor[OT::surface2P()];

    Array<plint,6> enlargeAry(2*dx0,-2*dx1,
                              2*dy0,-2*dy1,
                              2*dz0,-2*dz1);
    // this box represents the interface Coarse to fine, in fine units
    Box3D fineToCoarseBoxFineUnitsTmp   = coarseToFineBoxFineUnits.enlarge(overlapWidthCoarseUnits*enlargeAry);
    // this box represents the interface Coarse to fine, in coarse units
    ...
    for (plint iA = 0; iA < (plint)fineToCoarsePlanesFineUnitsTmp.size(); ++iA) {
        ...
        std::pair<plint,plint> dirOne = getDirections((dir+1) % 3);
        // Array<plint,2> nCellsOne(!neighbor[dirOne.first] && bcNeighbor[dirOne.first], !neighbor[dirOne.second] && bcNeighbor[dirOne.second]);
        Array<plint,2> nCellsOne(!neighbor[dirOne.first], !neighbor[dirOne.second]);

        std::pair<plint,plint> dirTwo = getDirections((dir+2) % 3);
        // Array<plint,2> nCellsTwo(!neighbor[dirTwo.first] && bcNeighbor[dirTwo.first], !neighbor[dirTwo.second] && bcNeighbor[dirTwo.second]);
        Array<plint,2> nCellsTwo(!neighbor[dirTwo.first], !neighbor[dirTwo.second]);
        std::pair<Array<plint,2>,Array<plint,2> > nEdge = getEdgesIndices(dir, ori, neighbor, bcNeighbor, 5, 4);


        Array<plint,6> bcAry(-!bcNeighbor[OT::surface0N()],!bcNeighbor[OT::surface0P()],
