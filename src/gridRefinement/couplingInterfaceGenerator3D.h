...
public:
    GridLevelContainer3D(Dynamics<T,Descriptor> *dyn_, const OctreeGridStructure& ogs, plint order_, plint level_);
...
public:
    ...
    MultiLevelCoupling3D(OctreeGridStructure& ogs_, Dynamics<T,Descriptor> *dyn_, plint order_,
                         plint overlapWidth_ = 1,
                         bool filterAll = true);
    ...
    void collideAndStream(plint iL, std::map<plint, bool> const& useExecuteInternalProcessors, 
        std::vector<plint> const &extProcIds, bool computeStats,
        plint statsId, std::vector<std::vector<plint > > const &ids,
...
private:
    void generateLevelsAndCouplingInterfaces();
